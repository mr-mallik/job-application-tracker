...
import { User, Briefcase, GraduationCap, Code, FolderOpen, Settings, RefreshCw } from 'lucide-react'
...
const navigation = [
  ...
  { name: 'Education & Certifications', href: '/profile/education', icon: GraduationCap },
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  item.destructive && !isActive && 'hover:bg-destructive/10 hover:text-destructive'
                )}
              >
                <Icon className={cn('h-4 w-4', item.destructive && !isActive && 'text-destructive/70')} />
                {item.name}
              </Link>
            )
          })}
