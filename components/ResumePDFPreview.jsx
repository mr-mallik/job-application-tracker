  ...
  const renderHeader = (header) => {
    ...
    return (
      <div className="text-center mb-4 pb-3 border-b-2 border-gray-400">
        ...
        <div
          className="flex items-center justify-center gap-3 mt-2 text-xs text-blue-600 flex-wrap"
          style={{ fontFamily: 'Inter, sans-serif' }}
        >
          {header.email && (
          ...
          {header.phone && (
            <>
              ...
              <a href={`tel:${header.phone}`} className="hover:underline">
                {header.phone}
              </a>
            </>
          )}
    ...
    <div className="mb-4">
      ...
      >
        {section.items.map((item, idx) => {
          const isSubheading =
            item.startsWith('**') || (item.includes('|') && !item.startsWith('-'));
          const isBullet = item.startsWith('-') || item.startsWith('•');

          if (isSubheading) {
            return (
              <p key={idx} className="font-semibold mt-2 text-sm">
                {item.replace(/\*\*/g, '')}
              </p>
            );
          } else if (isBullet) {
            return (
              <p key={idx} className="ml-3 text-gray-700">
                • {item.replace(/^[-•]\s*/, '')}
              </p>
            );
          } else {
