...
export async function sendEmail(to, subject, text, html) {
  if (!transporter) {
    ...
    console.log('📧 Email sending skipped (SMTP not configured). Email content:');
...
export async function updateUserProfile(userId, updates) {
  ...
  if (updates.jobSearchKeywords !== undefined)
    allowedUpdates.jobSearchKeywords = updates.jobSearchKeywords;
  if (updates.autoJobSearchEnabled !== undefined)
    allowedUpdates.autoJobSearchEnabled = updates.autoJobSearchEnabled;
  ...
  await users.updateOne({ id: userId }, { $set: allowedUpdates });
