...
import { slateFromText, slateToText, slateFromLines } from './slateUtils';
...
export const createDocHeaderBlock = (data = {}) =>
  createBlock(BLOCK_TYPES.DOC_HEADER, {
    ...
    designation: '',
    contact: [],
    links: [], // [{ label: string, url: string }] — structured link list
    ...data,
  });
...
export const createTextBlock = (text = '') =>
  createBlock(BLOCK_TYPES.TEXT, {
    text, // plain-text fallback for legacy/search
    slateContent: slateFromText(text), // rich-text value for editor
  });

export const createBulletBlock = (text = '') =>
  createBlock(BLOCK_TYPES.BULLET, {
    text,
    slateContent: slateFromText(text),
  });
  ...
  if (documentType === DOCUMENT_TYPES.RESUME) {
    ...
    blocks.push(
      createDocHeaderBlock({
        ...
        links: buildLinksArray(profile),
      })
    );

    // ── Professional Summary ─────────────────────────────────────────────
    if (profile?.summary) {
      blocks.push(createSectionTitleBlock('PROFESSIONAL SUMMARY'));
      blocks.push(createTextBlock(profile.summary));
    }

    // ── Work Experience ──────────────────────────────────────────────────
    const experiences = profile?.experiences || [];
    if (experiences.length > 0) {
      blocks.push(createSectionTitleBlock('WORK EXPERIENCE'));
      for (const exp of experiences) {
        ...
        // Render description lines as a single rich-text block with a bulleted list
        if (exp.description) {
          const lines = exp.description
            ...
            .filter(Boolean);
          if (lines.length > 0) {
            const block = createTextBlock(lines.join('\n'));
            block.data.slateContent = slateFromLines(lines);
            blocks.push(block);
          }
        }
      }
    ...
    if (education.length > 0) {
      blocks.push(createSectionTitleBlock('EDUCATION'));
      for (const edu of education) {
        ...
        if (edu.grade) {
          blocks.push(createTextBlock(`Grade: ${edu.grade}`));
        }
      }
    }
    ...
    if (skillGroups.length > 0) {
      blocks.push(createSectionTitleBlock('SKILLS'));
      for (const group of skillGroups) {
        blocks.push(
          createSkillGroupBlock({
            label: group.label,
            skills: group.items.map((s) => (typeof s === 'string' ? s : s.name)).filter(Boolean),
          })
        );
      }
    }
    ...
    if (projects.length > 0) {
      blocks.push(createSectionTitleBlock('PROJECTS'));
      for (const proj of projects) {
        ...
        if (proj.description) {
          blocks.push(createTextBlock(proj.description));
        }
      }
    }
    ...
    // ── Achievements ─────────────────────────────────────────────────────
    const achievements =
      typeof profile?.achievements === 'string' ? profile.achievements.trim() : '';
    if (achievements) {
      blocks.push(createSectionTitleBlock('ACHIEVEMENTS'));
    ...
    if (certifications.length > 0) {
      blocks.push(createSectionTitleBlock('CERTIFICATIONS'));
      for (const cert of certifications) {
        const name = typeof cert === 'string' ? cert : cert.name || cert.title || '';
        const issuer = cert.issuer || cert.institution || '';
...
// ─── Helper: build structured links array from profile ────────────────────

function buildLinksArray(profile) {
  if (!profile) return [];
  const links = [];
  ...
  if (profile.portfolio) {
    ...
    // Clean label: remove protocol and www
    const label = profile.portfolio.replace(/^https?:\/\/(www\.)?/, '');
    links.push({ label, url });
  }
  return links;
}

// ─── Runtime migration (client-side, non-destructive) ────────────────────

/**
 * Migrate a blocks array from any previous schema version to the current one.
 * Safe to call on every document load — idempotent.
 *
 * Changes applied:
 *  - TEXT / BULLET blocks without slateContent: derive from data.text
 *  - DOC_HEADER blocks without links array: add links: []
 */
  ...
  return blocks.map((block) => {
    ...
    switch (block.type) {
      ...
      case BLOCK_TYPES.BULLET:
        if (!block.data.slateContent) {
          return {
            ...block,
            data: {
              ...block.data,
              slateContent: slateFromText(block.data.text || ''),
            },
          };
        }
        return block;
      ...
      case BLOCK_TYPES.DOC_HEADER:
        if (!block.data.links) {
          return { ...block, data: { ...block.data, links: [] } };
        }
        return block;
  ...
  for (const block of blocks) {
    ...
    switch (block.type) {
      ...
      case BLOCK_TYPES.CL_SALUTATION: {
        const rich = block.data.slateContent
          ? slateToText(block.data.slateContent)
          : block.data.text;
