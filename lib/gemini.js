...
5. For CLOSING DATE - CRITICAL INSTRUCTIONS:
   - Look for phrases like: "closing date", "application deadline", "apply by", "deadline for applications", "applications close", "submit by", "closing on", "deadline:", "close date"
   - Check "Possible Dates" array first - the deadline is often there
   - Search the raw text for date patterns near deadline keywords
   - Return dates in YYYY-MM-DD format (e.g., "2026-04-15")
   - If you find dates like "15 April 2026", "April 15, 2026", "15/04/2026", or "04/15/2026" - convert to YYYY-MM-DD format
   - If only a partial date is given (e.g., "15th April" without year), assume current year 2026
   - Do NOT return dates that are job posting dates or start dates - only application deadline/closing dates
   - If absolutely no deadline is mentioned anywhere, return null
...
Return a valid JSON object with ONLY these fields:
{
  ...
  "closingDate": "Application deadline in YYYY-MM-DD format or null",
...
3. Find any application deadline or closing date - DETAILED INSTRUCTIONS:
   - Carefully scan the ENTIRE text for deadline-related keywords: "closing date", "application deadline", "apply by", "deadline for applications", "applications close", "submit by", "closing on", "deadline:", "close date", "expires", "final date"
   - Look in common sections: at the top, bottom, in bold text, near "how to apply" sections
   - Return dates in YYYY-MM-DD format (e.g., "2026-04-15")
   - Convert any date format to YYYY-MM-DD:
     ...
     * "15th Apr 2026" → "2026-04-15"
   - If only day/month given (e.g., "15th April"), assume current year 2026
   - Do NOT confuse with job posting date or start date - ONLY extract application deadline
   - If absolutely no deadline mentioned, return null
...
RULES:
1. Extract dates in mm/yyyy format using numerical month and year (e.g., 09/2022 for September 2022).
2. Use "Present" for current positions (case-sensitive)
3. For education, use both startDate and endDate in mm/yyyy format, plus graduationDate
...
${userProfile ? JSON.stringify(userProfile) : 'No profile data provided'}
...
PROFESSIONAL PROFILE → EDUCATION → SKILLS → PROJECTS → WORK EXPERIENCE → CERTIFICATIONS → ACHIEVEMENTS → INTERESTS

MID-LEVEL (2–8 years):
PROFESSIONAL PROFILE → WORK EXPERIENCE → EDUCATION → SKILLS → CERTIFICATIONS → PROJECTS → ACHIEVEMENTS →INTERESTS

SENIOR (8–15 years):
PROFESSIONAL PROFILE → WORK EXPERIENCE → KEY ACHIEVEMENTS → EDUCATION → SKILLS → CERTIFICATIONS → INTERESTS
...
- Date format: Profile dates are stored as "mm/yyyy" (e.g., 08/2024, 11/2023). Convert to display format "Month Year" (e.g., Sep 2024, Mar 2023) by removing the slash
...
Use only these block types:
  ...
  { "type": "subheading",    "data": { "primary": "Job Title", "secondary": "Company Name, Location", "location": "", "dateRange": "Month Year – Month Year" } }
...
- subheading.dateRange: Convert profile dates from "09/2024" format to display format "Sep 2024" by removing the slash. Examples: "09/2022 – 03/2024" becomes "Sep 2022 – Mar 2024", "04/2025 – Present" becomes "Apr 2025 – Present"
- Return only the JSON array, nothing else
`;
    writeLog('Generating resume blocks with Gemini AI for resume:', prompt);
