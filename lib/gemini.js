...
5. Output markdown format with NO header/contact section
6. Do NOT add commentary, explanations, or notes outside the CV content itself
`;
...
Output the cover letter in markdown with bold for the reference line and any key emphasis. Paragraph breaks between each section.
`;
...
Output the supporting statement in markdown, with **bold** criterion headings, proper paragraph breaks, and bullet points where they improve clarity.
`;
  }
...
/**
 * AI-powered document refinement that returns block JSON instead of raw markdown.
 * Chains refineDocument (markdown) → markdownToBlocks (block[]).
 *
 * @param {string} documentType - 'resume' | 'coverLetter' | 'supportingStatement'
 * @param {string} content - existing markdown content or empty
 * @param {string} jobDescription - job description text
 * @param {string} userPreferences - optional extra instructions
 * @param {object|null} userProfile - structured user profile
 * @returns {Promise<Array>} blocks array conforming to blockSchema
 */
export async function refineDocumentToBlocks(
  ...
  const markdown = await refineDocument(
    documentType,
    content,
    jobDescription,
    userPreferences,
  ...
  return markdownToBlocks(markdown, documentType);
}
