...
- Date format: Month Year (e.g., March 2023 — NOT 03/2023 or Mar '23)
...
- subheading.dateRange must use full month names: "March 2022 – Present" (not 03/2022)
...
/**
 * Analyse a resume's block content against a job description and extract
 * keyword coverage data.
 *
 * Returns:
 *   {
 *     keywords: [{ keyword: string, present: boolean, context?: string }],
 *     score: number,   // 0-100
...
Analyse the resume text against the job description below and identify the key skills, qualifications, tools, technologies, and role-specific terms that the employer is looking for.
...
2. For each keyword, determine whether it appears (or a close synonym/variant appears) in the resume text.
3. Compute an overall coverage score as a percentage (keywords present / total keywords × 100), rounded to the nearest integer.
4. Write a 1–2 sentence plain text summary of the coverage quality and any critical missing areas.

Return ONLY a valid JSON object in exactly this shape, no markdown, no code fences:
{
  "keywords": [
    { "keyword": "Python", "present": true },
    { "keyword": "stakeholder management", "present": false }
  ],
  "score": 72,
  "summary": "Your resume covers most technical requirements but is missing key leadership and stakeholder management terms."
}
`;
  ...
  try {
    ...
    throw new Error('Failed to analyse keywords');
  }
}
