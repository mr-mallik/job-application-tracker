...
export function renderPDFBlock(block, styles, options = {}) {
  ...
  switch (type) {
    ...
    case BLOCK_TYPES.SUBHEADING:
      return (
        <View key={id} style={styles.itemContainer || {}}>
          {data.primary ? <Text style={styles.subheading}>{data.primary}</Text> : null}
          {data.secondary || data.location || data.dateRange ? (
            <Text style={styles.text || {}}>
              {[data.secondary, data.location, data.dateRange].filter(Boolean).join(' | ')}
            </Text>
          ) : null}
        </View>
      );
    ...
    case BLOCK_TYPES.TEXT:
      return data.text ? (
        <Text key={id} style={styles.text || styles.paragraph || {}}>
          {data.text}
        </Text>
      ) : null;
    ...
    case BLOCK_TYPES.BULLET:
      return data.text ? (
        <View key={id} style={styles.bulletContainer || {}}>
          ...
          <Text style={styles.bulletText || styles.bulletContent || {}}>{data.text}</Text>
        </View>
      ) : null;
    ...
    case BLOCK_TYPES.SKILL_GROUP: {
      ...
      // Plain text fallback (ATS)
      return (
        <View key={id} style={styles.skillsContainer || {}}>
          {data.label ? (
            <Text style={styles.subheading || styles.text || {}}>{data.label}</Text>
          ) : null}
          <Text style={styles.text || styles.skill || {}}>{data.skills.join(', ')}</Text>
        </View>
      );
    }
