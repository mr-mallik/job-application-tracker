...
function validateJobContent(html, visibleText) {
  ...
  if (hasAuthIndicators && visibleText.length < 1000) {
    return {
      valid: false,
      reason:
        'Login or authentication page detected. URL may require authentication or is not a direct job posting link.',
    };
  }
  ...
  const jobIndicatorCount = jobIndicators.filter(
    (indicator) => textLower.includes(indicator) || htmlLower.includes(indicator)
  ).length;
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (is429 && !isLastAttempt) {
        ...
        console.log(
          `[Retry] Rate limited, waiting ${delay}ms before retry ${attempt}/${maxRetries}`
        );
...
export async function scrapeWithPlaywright(url) {
  ...
  if (requiresAdvancedStealth(url)) {
    try {
      const result = await retryWithBackoff(() => scrapeWithBrowserQL(url, BROWSERLESS_API_KEY));
  ...
  // Wrap main scraping logic in retry helper
  return await retryWithBackoff(
    async () => {
      ...
      $(
        'script, style, nav, footer, header, aside, iframe, noscript, [role="navigation"], [role="banner"]'
      ).remove();
    ...
    3,
    3000
  ); // 3 retries, starting with 3s delay
}
...
async function scrapeWithContentEndpoint(url, apiKey) {
  return await retryWithBackoff(
    ...
    3,
    3000
  ); // 3 retries with 3s initial delay
}
...
export function parseWithCheerio(html, url) {
  ...
  if (url.includes('linkedin.com')) {
    extractedData.possibleTitles.push(
    ...
    extractedData.possibleCompanies.push(
    ...
    extractedData.possibleLocations.push(
  ...
  if (url.includes('indeed.com')) {
    ...
    extractedData.possibleCompanies.push(
      $('.jobsearch-InlineCompanyRating-companyName').text().trim()
    );
    extractedData.possibleCompanies.push(
    ...
    extractedData.possibleLocations.push(
      $('.jobsearch-JobInfoHeader-subtitle').find('div').eq(1).text().trim()
    );
  ...
  $(
    '[class*="company"], [class*="Company"], [class*="employer"], [class*="Employer"], [class*="organisation"], [class*="organization"]'
  ).each((_, el) => {
    const text = $(el).text().trim();
  ...
  // Locations
  $('[class*="location"], [class*="Location"], [class*="place"], [class*="address"]').each(
    (_, el) => {
      const text = $(el).text().trim();
  ...
  $('[class*="salary"], [class*="Salary"], [class*="pay"], [class*="compensation"]').each(
    (_, el) => {
      const text = $(el).text().trim();
  ...
  const salaryPatterns =
    /(?:£|€|\$|USD|GBP|EUR)\s*[\d,]+(?:\s*-\s*(?:£|€|\$|USD|GBP|EUR)?\s*[\d,]+)?(?:\s*(?:per|\/|p\.?a\.?|annually|yearly|monthly|hourly|pa|k))?/gi;
  ...
  $(
    '[class*="description"], [class*="Description"], [class*="details"], [class*="content"], article, main, [role="main"]'
  ).each((_, el) => {
    const text = $(el).text().trim();
