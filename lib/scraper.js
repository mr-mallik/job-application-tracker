...
function normalizeLinkedInUrl(url) {
  try {
    ...
    if (urlObj.hostname.includes('linkedin.com') && 
      ...
      console.log('[LinkedIn] URL normalized:')
...
async function scrapeWithBrowserQL(url, apiKey) {
  console.log('[BrowserQL] Using GraphQL API with stealth mode...')
  ...
  console.log('[BrowserQL] Navigation status:', data.goto?.status)
  console.log('[BrowserQL] Show more clicked:', data.clickShowMore?.success)
  
  // Extract HTML and text from BrowserQL response
  const html = data.html?.html || ''
  const visibleText = data.fullContent?.text || ''
  
  console.log('[BrowserQL] HTML length:', html.length)
  console.log('[BrowserQL] Text length:', visibleText.length)
  ...
  console.log('[BrowserQL] Structured data:', JSON.stringify(structuredData, null, 2))
...
export async function scrapeWithPlaywright(url) {
  ...
  if (requiresAdvancedStealth(url)) {
    console.log('[Scraper] Protected site detected, trying BrowserQL with stealth mode...')
    try {
      ...
      if (validation.valid) {
        console.log('[BrowserQL] ✓ Successfully scraped with stealth mode')
        return result
      } else {
        console.warn('[BrowserQL] Validation failed:', validation.reason)
        console.log('[BrowserQL] Falling back to REST API...')
      }
    } catch (error) {
  ...
  try {
    console.log('[Browserless] Scraping with /scrape REST API (Chrome)...')
            ...
            setTimeout(() => {
              ...
              expandButtons.forEach(btn => {
                ...
                if (text.includes('show more') || text.includes('see more') || 
                  ...
                  try { 
                    btn.click(); 
                    console.log('Clicked expand button:', text.substring(0, 30));
                  } catch(e) {}
                }
              });
            }, 1500);
    ...
    if (!response.ok) {
      ...
      console.log('[Browserless] Falling back to /content endpoint...')
      return await scrapeWithContentEndpoint(url, BROWSERLESS_API_KEY)
    }
    
    const data = await response.json()
    console.log('[Browserless] Scrape API returned data:', Object.keys(data))
    ...
    console.log('[Browserless] Full HTML fetched, size:', html.length)
    ...
    console.log('[Browserless] Visible text extracted, length:', visibleText.length)
    ...
    console.log('[Browserless] Content validated successfully')
...
async function scrapeWithContentEndpoint(url, apiKey) {
  console.log('[Browserless] Using /content endpoint...')
