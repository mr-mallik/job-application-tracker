/**
 * Slate.js utility functions for block-based rich text editing.
 *
 * Slate value format used in this app (single-paragraph per block):
 *
 *   [
 *     {
 *       type: 'paragraph',
 *       children: [
 *         { text: 'Hello ' },
 *         { text: 'world', bold: true },
 *         { type: 'link', url: 'https://...', children: [{ text: 'click here' }] }
 *       ]
 *     }
 *   ]
 *
 * Leaf marks: bold, italic, underline
 * Inline elements: link (type:'link', url: string)
 */

import { Range } from 'slate';

export const SLATE_ELEM = {
  PARAGRAPH: 'paragraph',
  LINK: 'link',
  BULLETED_LIST: 'bulleted-list',
  LIST_ITEM: 'list-item',
};

// Helper to get the correct bold font variant based on the base font family
function getBoldFont(baseFontFamily) {
  if (!baseFontFamily || baseFontFamily === 'Helvetica') return 'Helvetica-Bold';
  if (baseFontFamily === 'Times-Roman') return 'Times-Bold';
  if (baseFontFamily === 'Courier') return 'Courier-Bold';
  return 'Helvetica-Bold'; // fallback
}

/**
 * Parse markdown bold syntax (**text**) within a text string and return array of Text components
 * @param {string} text - Text potentially containing **bold** markdown
 * @param {object} baseStyle - Base text style
 * @param {string} boldFont - Bold font family
 * @param {Component} TextComp - React PDF Text component
 * @param {string} keyPrefix - Unique key prefix for React elements
 * @returns {Array<Component>} - Array of Text components
 */
function parseTextWithBold(text, baseStyle, boldFont, TextComp, keyPrefix) {
  const parts = [];
  let currentIndex = 0;

  const boldPattern = /\*\*([^*]+)\*\*/g;
  let match;
  let matchCount = 0;

  while ((match = boldPattern.exec(text)) !== null) {
    // Add text before the match
    if (match.index > currentIndex) {
      const beforeText = text.substring(currentIndex, match.index);
      parts.push(
        <TextComp key={`${keyPrefix}-text-${matchCount}`} style={baseStyle}>
          {beforeText}
        </TextComp>
      );
    }

    // Add bold text
    parts.push(
      <TextComp
        key={`${keyPrefix}-bold-${matchCount}`}
        style={{ ...baseStyle, fontFamily: boldFont }}
      >
        {match[1]}
      </TextComp>
    );

    currentIndex = match.index + match[0].length;
    matchCount++;
  }

  // Add remaining text after last match
  if (currentIndex < text.length) {
    parts.push(
      <TextComp key={`${keyPrefix}-text-end`} style={baseStyle}>
        {text.substring(currentIndex)}
      </TextComp>
    );
  }

  return parts.length > 0
    ? parts
    : [
        <TextComp key={keyPrefix} style={baseStyle}>
          {text}
        </TextComp>,
      ];
}

// ─── Value creation & validation ──────────────────────────────────────────

/** Create a minimal Slate value from a plain text string */
export function slateFromText(text = '') {
  return [{ type: SLATE_ELEM.PARAGRAPH, children: [{ text: text || '' }] }];
}

/** Create a Slate bulleted-list value from an array of plain-text lines */
export function slateFromLines(lines = []) {
  if (!lines || lines.length === 0) return slateFromText('');
  return [
    {
      type: SLATE_ELEM.BULLETED_LIST,
      children: lines.map((line) => ({
        type: SLATE_ELEM.LIST_ITEM,
        children: [{ text: line }],
      })),
    },
  ];
}

/** Return true if value looks like a Slate document */
export function isSlateValue(value) {
  return Array.isArray(value) && value.length > 0 && value[0]?.type !== undefined;
}

/** Coerce anything (string, null, Slate array) to a valid Slate value */
export function ensureSlate(value) {
  if (isSlateValue(value)) return value;
  if (typeof value === 'string') return slateFromText(value);
  return slateFromText('');
}

// ─── Serialisation ────────────────────────────────────────────────────────

/** Extract plain text from a Slate value (for AI, search, PDF fallback) */
export function slateToText(value) {
  if (!isSlateValue(value)) {
    return typeof value === 'string' ? value : '';
  }
  return value.map(_nodeText).join('\n');
}

function _nodeText(node) {
  if (typeof node.text === 'string') return node.text;
  if (node.type === SLATE_ELEM.BULLETED_LIST && Array.isArray(node.children)) {
    return node.children.map((item) => '\u2022 ' + _nodeText(item)).join('\n');
  }
  if (Array.isArray(node.children)) return node.children.map(_nodeText).join('');
  return '';
}

/**
 * Serialize a Slate value to an array of @react-pdf/renderer Text / Link elements.
 * The caller provides the PDF component references to avoid importing @react-pdf here
 * (it can't run server-side).
 *
 * Usage inside renderPDFBlock:
 *   const spans = slateToPdfSpans(data.slateContent, styles.text, styles.link, Text, Link);
 *   return <Text style={styles.text}>{spans}</Text>
 */
export function slateToPdfSpans(value, baseStyle, linkStyle, TextComp, LinkComp) {
  if (!isSlateValue(value)) return null;

  const spans = [];
  const baseFontFamily = baseStyle?.fontFamily || 'Helvetica';
  const boldFont = getBoldFont(baseFontFamily);

  value.forEach((para, pIdx) => {
    if (pIdx > 0) {
      // paragraph break as newline
      spans.push(
        <TextComp key={`br-${pIdx}`} style={baseStyle}>
          {'\n'}
        </TextComp>
      );
    }

    // ── Bulleted list ────────────────────────────────────────────────────
    if (para.type === SLATE_ELEM.BULLETED_LIST && Array.isArray(para.children)) {
      para.children.forEach((item, iIdx) => {
        if (iIdx > 0) {
          spans.push(
            <TextComp key={`blbr-${pIdx}-${iIdx}`} style={baseStyle}>
              {'\n'}
            </TextComp>
          );
        }
        spans.push(
          <TextComp key={`bull-${pIdx}-${iIdx}`} style={baseStyle}>
            {'\u2022 '}
          </TextComp>
        );
        (item.children || []).forEach((child, cIdx) => {
          const key = `blic-${pIdx}-${iIdx}-${cIdx}`;
          if (child.type === SLATE_ELEM.LINK) {
            const txt = (child.children || []).map(_nodeText).join('');
            if (txt)
              spans.push(
                <LinkComp key={key} src={child.url || '#'} style={{ ...baseStyle, ...linkStyle }}>
                  {txt}
                </LinkComp>
              );
            return;
          }
          const style = {
            ...baseStyle,
            ...(child.bold ? { fontFamily: boldFont } : {}),
            ...(child.italic ? { fontStyle: 'italic' } : {}),
            ...(child.underline ? { textDecoration: 'underline' } : {}),
          };
          // Only add Text component if there's actual text content
          if (child.text) {
            // Check for markdown bold syntax (**text**)
            if (child.text.includes('**') && !child.bold) {
              // Parse markdown bold - use baseStyle with italic/underline but not pre-applied bold
              const styleWithoutBold = {
                ...baseStyle,
                ...(child.italic ? { fontStyle: 'italic' } : {}),
                ...(child.underline ? { textDecoration: 'underline' } : {}),
              };
              const parsed = parseTextWithBold(
                child.text,
                styleWithoutBold,
                boldFont,
                TextComp,
                key
              );
              spans.push(...parsed);
            } else {
              spans.push(
                <TextComp key={key} style={style}>
                  {child.text}
                </TextComp>
              );
            }
          }
        });
      });
      return; // skip normal paragraph processing
    }

    if (!Array.isArray(para.children)) return;

    para.children.forEach((child, cIdx) => {
      const key = `p${pIdx}c${cIdx}`;

      if (child.type === SLATE_ELEM.LINK) {
        const txt = Array.isArray(child.children) ? child.children.map(_nodeText).join('') : '';
        if (txt) {
          spans.push(
            <LinkComp key={key} src={child.url || '#'} style={{ ...baseStyle, ...linkStyle }}>
              {txt}
            </LinkComp>
          );
        }
        return;
      }

      // Regular leaf
      const style = {
        ...baseStyle,
        ...(child.bold ? { fontFamily: boldFont } : {}),
        ...(child.italic ? { fontStyle: 'italic' } : {}),
        ...(child.underline ? { textDecoration: 'underline' } : {}),
      };
      // Only add Text component if there's actual text content
      if (child.text) {
        // Check for markdown bold syntax (**text**)
        if (child.text.includes('**') && !child.bold) {
          // Parse markdown bold - use baseStyle with italic/underline but not pre-applied bold
          const styleWithoutBold = {
            ...baseStyle,
            ...(child.italic ? { fontStyle: 'italic' } : {}),
            ...(child.underline ? { textDecoration: 'underline' } : {}),
          };
          const parsed = parseTextWithBold(child.text, styleWithoutBold, boldFont, TextComp, key);
          spans.push(...parsed);
        } else {
          spans.push(
            <TextComp key={key} style={style}>
              {child.text}
            </TextComp>
          );
        }
      }
    });
  });

  return spans;
}

// ─── Slate editor helpers (used in RichTextEditor component) ──────────────

/** Check if a given format mark is active on the current selection */
export function isMarkActive(editor, format) {
  const marks = editor.getMarks?.() ?? {};
  return marks[format] === true;
}

/** Toggle a mark on / off */
export function toggleMark(editor, format) {
  if (isMarkActive(editor, format)) {
    editor.removeMark(format);
  } else {
    editor.addMark(format, true);
  }
}

/** Check if a link element wraps the current selection */
export function isLinkActive(editor) {
  const { selection } = editor;
  if (!selection) return false;
  const [match] = Array.from(
    editor.nodes({ at: selection, match: (n) => n.type === SLATE_ELEM.LINK })
  );
  return !!match;
}

/** Unwrap any link wrapping the current selection */
export function unwrapLink(editor) {
  editor.unwrapNodes({ match: (n) => n.type === SLATE_ELEM.LINK });
}

/** Check if the current selection is inside a bulleted list */
export function isListActive(editor) {
  const { selection } = editor;
  if (!selection) return false;
  const [match] = Array.from(
    editor.nodes({ at: selection, match: (n) => n.type === SLATE_ELEM.BULLETED_LIST })
  );
  return !!match;
}

/** Toggle a bulleted list on/off for the current selection */
export function toggleList(editor) {
  if (isListActive(editor)) {
    // Lift items out, convert list-items back to paragraphs
    editor.unwrapNodes({ match: (n) => n.type === SLATE_ELEM.BULLETED_LIST, split: true });
    editor.setNodes(
      { type: SLATE_ELEM.PARAGRAPH },
      { match: (n) => n.type === SLATE_ELEM.LIST_ITEM }
    );
  } else {
    // Convert paragraphs to list-items, then wrap in a bulleted-list
    editor.setNodes({ type: SLATE_ELEM.LIST_ITEM });
    editor.wrapNodes({ type: SLATE_ELEM.BULLETED_LIST, children: [] });
  }
}

/** Wrap current selection in a link with the given url */
export function wrapLink(editor, url) {
  if (isLinkActive(editor)) unwrapLink(editor);

  const { selection } = editor;
  const isCollapsed = selection && Range.isCollapsed(selection);

  const link = {
    type: SLATE_ELEM.LINK,
    url,
    children: isCollapsed ? [{ text: url }] : [],
  };

  if (isCollapsed) {
    editor.insertNodes(link);
  } else {
    editor.wrapNodes(link, { split: true });
    editor.collapse({ edge: 'end' });
  }
}
