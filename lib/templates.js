...
export function generateCoverLetterTemplate(profile, job) {
  ...
  // Salutation
  const salutation = job?.hiringManager ? `Dear ${job.hiringManager},` : 'Dear Hiring Manager,';
  markdown += `${salutation}\n\n`;
  ...
  if (profile?.skills?.technical && Array.isArray(profile.skills.technical)) {
    ...
    skillsList = profile.skills.relevant
      .split(',')
      .slice(0, 3)
      .map((s) => s.trim());
  }
...
export function generateSupportingStatementTemplate(profile, job) {
  ...
  if (profile?.skills?.technical && Array.isArray(profile.skills.technical)) {
    ...
    technicalSkills = profile.skills.relevant
      .split(',')
      .slice(0, 5)
      .map((s) => s.trim());
  }
