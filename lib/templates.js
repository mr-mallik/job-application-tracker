...
export function generateCoverLetterTemplate(profile, job) {
  ...
  if (profile?.headline) {
    markdown += `As a ${profile.headline.toLowerCase()}, I am excited about the opportunity to contribute to your team.\n\n`
  } else {
  ...
  if (profile?.experiences && profile.experiences.length > 0) {
    ...
    markdown += `- ${exp.designation || 'Professional role'} at ${exp.company || 'previous organization'}\n`
  }
  
  if (profile?.skills?.technical && profile.skills.technical.length > 0) {
    markdown += `- Technical skills including ${profile.skills.technical.slice(0, 3).map(s => s.name).join(', ')}\n`
  }
...
export function generateSupportingStatementTemplate(profile, job) {
  ...
  if (profile?.experiences && profile.experiences.length > 0) {
    ...
    const exp = profile.experiences[0]
    markdown += `- ${exp.designation || 'Current role'} at ${exp.company || 'company'}\n`
    if (exp.description) {
  ...
  if (profile?.skills?.technical && profile.skills.technical.length > 0) {
    ...
    profile.skills.technical.slice(0, 5).forEach(skill => {
